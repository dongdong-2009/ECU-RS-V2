...
int Write_ECUID(char *ECUID)		//ECU ID  12×Ö½Ú
{
	char ecuid[13] = {'\0'};
...
int Read_ECUID(char *ECUID)		//¶ÁÈ¡ECUID  12×Ö½Ú
{
	int fd;
...
int Write_IO_INIT_STATU(char *IO_InitStatus)								//IOÉÏµç×´Ì¬
{
...
int Write_WIFI_PW(char *WIFIPasswd,unsigned char Counter)//WIFIÃÜÂë
{
	int fd;
...
//Ä¿Â¼¼ì²â£¬Èç¹û²»´æÔÚ¾Í´´½¨¶ÔÓÚµÄÄ¿Â¼
void dirDetection(char *path)
{
	DIR *dirp;

	//ÏÈÅÐ¶ÏÊÇ·ñ´æÔÚÕâ¸öÄ¿Â¼£¬Èç¹û²»´æÔÚÔò´´½¨¸ÃÄ¿Â¼
	dirp = opendir(path);
	if(dirp == RT_NULL){	//´ò¿ªÄ¿Â¼Ê§°Ü£¬ÐèÒª´´½¨¸ÃÄ¿Â¼
		mkdir(path,0);
	}else{					//´ò¿ªÄ¿Â¼³É¹¦£¬¹Ø±ÕÄ¿Â¼¼ÌÐø²Ù×÷
		closedir(dirp);
	}
}
...
void init_tmpdb(inverter_info *firstinverter)
{
	...
	if(fp)
	{
		while(NULL != fgets(data,200,fp))
		{
			...
			//ÅÐ¶ÏÊÇ·ñ´æÔÚ¸ÃÄæ±äÆ÷
			//½«12Î»µÄRSD ID×ª»»Îª6Î»µÄBCD±àÂëID
...
int delete_line(char* filename,char* temfilename,char* compareData,int len)
{
	FILE *fin,*ftp;
  char data[512] = {'\0'};
  fin=fopen(filename,"r");//ÒÔÖ»¶Á·½Ê½´ò¿ªÎÄ¼þ1
	if(fin == NULL)
	{
  ...
  while(fgets(data,512,fin))//¶ÁÈ¡ÐÐÊý¾Ý
	{
		if(memcmp(data,compareData,len))
		{
			//print2msg(ECU_DBG_OTHER,"delete_line",data);
			fputs(data,ftp);//ÊäÈë·ûºÏ±ê×¼µÄÊý¾Ýµ½ÎÄ¼þ1
		}
	}
  fclose(fin);
  fclose(ftp);
  remove(filename);//É¾³ýÔ­À´µÄÎÄ¼þ
  rename(temfilename,filename);//½«ÁÙÊ±ÎÄ¼þ¸ÄÃûÎªÔ­À´µÄÎÄ¼þ
  return 0;
...
int search_line(char* filename,char* compareData,int len)
{
  ...
  while(fgets(data,300,fin))//¶ÁÈ¡300¸ö×Ö½Ú
	{
		if(!memcmp(data,compareData,len))
		{
			//¶ÁÈ¡µ½ÁË¹Ø±ÕÎÄ¼þ£¬·µ»Ø1 ±íÊ¾´æÔÚ¸ÃÐÐÊý¾Ý¡Œ
			fclose(fin);
			return 1;
		}
	}
...
//¼ì²éÄ¿Â¼ÖÐÊ±¼ä×îÔçµÄÎÄ¼þ,´æÔÚ·µ»Ø1£¬²»´æÔÚ·µ»Ø0
//dirÎªÄ¿Â¼µÄÃû³Æ£¨´«Èë²ÎÊý£©    oldfileÎª×îÔçÎÄ¼þµÄÎÄ¼þÃû£¨´«³ö²ÎÊý£©
...
//¼ìË÷Õû¸öÎÄ¼þÏµÍ³£¬ÅÐ¶ÏÊ£Óà¿Õ¼ä´æ´¢Á¿£¬Èç¹ûÊ£Óà¿É´æ´¢¿Õ¼ä¹ýÐ¡£¬Ôò¼ìË÷ÏàÓ¦µÄÄ¿Â¼£¬²¢½øÐÐÏàÓ¦µÄÉ¾³ý²Ù×÷
int optimizeFileSystem(int capsize)
{
  ...
  cap = buffer.f_bsize * buffer.f_bfree / 1024;
	...
	//µ±flashÐ¾Æ¬ËùÊ£ÏÂµÄÈÝÁ¿Ð¡ÓÚ40KBµÄÊ±ºò½øÐÐÒ»Ð©±ØÒªµÄÎÄ¼þÉ¾³ý²Ù×÷¡£
	if (cap < capsize) 
	{
		//É¾³ý×îÇ°ÃæÒ»ÌìµÄECU¼¶±ð´¦Àí½á¹ûÊý¾Ý    Èç¹û¸ÃÄ¿Â¼ÏÂ´æÔÚÎÄ¼þµÄ»°
		if(1 == checkOldFile("/home/data/proc_res",oldFile))
		{
			unlink(oldFile);
		}
		
		//É¾³ý×îÇ°ÃæÒ»ÌìµÄÄæ±äÆ÷¼¶±ð´¦Àí½á¹ûÊý¾Ý  Èç¹û¸ÃÄ¿Â¼ÏÂ´æÔÚÎÄ¼þµÄ»°
		...
		//É¾³ý×îÇ°ÃæÒ»ÌìµÄÄæ±äÆ÷×´Ì¬Êý¾Ý Èç¹û¸ÃÄ¿Â¼ÏÂ´æÔÚÎÄ¼þµÄ»°
		memset(oldFile,0x00,100);
...
//·µ»Ø0±íÊ¾DHCPÄ£Ê½  ·µ»Ø1±íÊ¾¾²Ì¬IPÄ£Ê½
...
void updateID(void)
{
	...
	if(fp)
	{
		...
		for(i=0; (i<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); i++, curinverter++)			//ÓÐÐ§Äæ±äÆ÷ÂÖÑµ
		{
...
int get_id_from_file(inverter_info *firstinverter)
{
	...
	if(fp)
	{
		while(NULL != fgets(data,200,fp))
		{
			...
			if(0==strlen(list[2]))
			{
				inverter->model = 0xff;		//Î´»ñµÃ»úÐÍÂëµÄÄæ±äÆ÷¸³ÖµÎª0.ZK
			}
			else
			{
			...
			if(0==strlen(list[4]))
			{
				inverter->status.bindflag = 0;		//Î´°ó¶¨µÄÄæ±äÆ÷°Ñ±êÖ¾Î»¸³ÖµÎª0.ZK
			}
			else
			{
...
int calculate_earliest_2_day_ago(char *date,int *earliest_data)
{
	...
	char month_s[3] = {'\0'};
	char day_s[3] = {'\0'};
	int year = 0,month = 0,day = 0;	//yearÎªÄê·Ý monthÎªÔÂ·Ý dayÎªÈÕÆÚ  flag ÎªÈðÄêÅÐ¶Ï±êÖ¾ count±íÊ¾ÉÏ¸öÔÂºÃÐèÒª²¹µÄÌìÊý number_of_days:ÉÏ¸öÔÂµÄ×ÜÌìÊý
	int flag = 0;
...
int calculate_earliest_2_month_ago(char *date,int *earliest_data)
{
	char year_s[5] = {'\0'};
	char month_s[3] = {'\0'};
	int year = 0,month = 0;	//yearÎªÄê·Ý monthÎªÔÂ·Ý dayÎªÈÕÆÚ  flag ÎªÈðÄêÅÐ¶Ï±êÖ¾ count±íÊ¾ÉÏ¸öÔÂºÃÐèÒª²¹µÄÌìÊý number_of_days:ÉÏ¸öÔÂµÄ×ÜÌìÊý
...
int read_system_power(char *date_time, char *power_buff,int *length)
{
	...
	*length = 0;
	...
	if(fp)
	{
		while(NULL != fgets(buff,100,fp))  //¶ÁÈ¡Ò»ÐÐÊý¾Ý
		{	//µÚ8¸ö×Ö½ÚÎª£¬  ÇÒÊ±¼äÏàÍ¬
...
void update_daily_energy(float current_energy, char *date_time)
{
	...
	file[6] = '\0';
	sprintf(dir,"%s%s.dat",dir,file);
	date_time_tmp[8] = '\0';	//´æ´¢Ê±¼äÎªÄêÔÂÈÕ ÀýÈç:20170718

	//¼ì²âÊÇ·ñÒÑ¾­´æÔÚ¸ÃÊ±¼äµãµÄÊý¾Ý
...
//¼ÆËãÒ»ÖÜÖÐ×îÔçÒ»ÌìµÄÊ±¼ä		·µ»ØÖµ£º0±íÊ¾×îÔçÒ»ÌìÔÚµ±ÔÂ   1£º±íÊ¾×îÔçÒ»ÌìÔÚÉÏ¸öÔÂ
int calculate_earliest_week(char *date,int *earliest_data)
{
	char year_s[5] = {'\0'};
	char month_s[3] = {'\0'};
	char day_s[3] = {'\0'};
	int year = 0,month = 0,day = 0,flag = 0,count = 0;	//yearÎªÄê·Ý monthÎªÔÂ·Ý dayÎªÈÕÆÚ  flag ÎªÈðÄêÅÐ¶Ï±êÖ¾ count±íÊ¾ÉÏ¸öÔÂºÃÐèÒª²¹µÄÌìÊý number_of_days:ÉÏ¸öÔÂµÄ×ÜÌìÊý
...
int read_weekly_energy(char *date_time, char *power_buff,int *length)
{
	...
	*length = 0;
	//¼ÆËãÇ°ÆßÌìÖÐ×îÔçµÄÒ»Ìì   Èç¹ûflagÎª0  ±íÊ¾×îÔçÒ»ÌìÔÚµ±ÔÂ  Èç¹ûÎª1±íÊ¾ÔÚÉÏ¸öÔÂ
...
int calculate_earliest_month(char *date,int *earliest_data)
{
	...
	{
		//Èç¹ûÐ¡ÓÚ28ºÅ£¬È¡ÉÏ¸öÔÂµÄ¸ÃÌìµÄºóÒ»Ìì 
		day = day + 1;
		month = month - 1;
		if(month == 0)
		{
...
void update_monthly_energy(float current_energy, char *date_time)
{
	...
	date_time_tmp[6] = '\0';	//´æ´¢Ê±¼äÎªÄêÔÂÈÕ ÀýÈç:20170718
...
void update_yearly_energy(float current_energy, char *date_time)
{
	...
	date_time_tmp[4] = '\0';	//´æ´¢Ê±¼äÎªÄê ÀýÈç:2017
...
int detection_resendflag2(void)		//´æÔÚ·µ»Ø1£¬²»´æÔÚ·µ»Ø0
{
	DIR *dirp;
				...
				if(fp)
				{
					while(NULL != fgets(buff,(CLIENT_RECORD_HEAD+CLIENT_RECORD_ECU_HEAD+CLIENT_RECORD_INVERTER_LENGTH*MAXINVERTERCOUNT+CLIENT_RECORD_OTHER),fp))
					{
						if(strlen(buff) > 18)
						{
							...
							{
								if(buff[strlen(buff)-2] == '2')			//¼ì²â×îºóÒ»¸ö×Ö½ÚµÄresendflagÊÇ·ñÎª2   Èç¹û·¢ÏÖÊÇ2  ¹Ø±ÕÎÄ¼þ²¢ÇÒreturn 1
								{
									fclose(fp);
									closedir(dirp);
									free(buff);
...
int change_resendflag(char *time,char flag)  //¸Ä±ä³É¹¦·µ»Ø1£¬Î´ÕÒµ½¸ÃÊ±¼äµã·µ»Ø0
{
	DIR *dirp;
	char dir[30] = "/home/record/data";
	struct dirent *d;
...
//²éÑ¯Ò»ÌõresendflagÎª1µÄÊý¾Ý   ²éÑ¯µ½ÁË·µ»Ø1  Èç¹ûÃ»²éÑ¯µ½·µ»Ø0
/*
data:±íÊ¾»ñÈ¡µ½µÄÊý¾Ý
time£º±íÊ¾»ñÈ¡µ½µÄÊ±¼ä
flag£º±íÊ¾ÊÇ·ñ»¹ÓÐÏÂÒ»ÌõÊý¾Ý   ´æÔÚÏÂÒ»ÌõÎª1   ²»´æÔÚÎª0
*/
		...
		else
		{
			...
			while ((d = readdir(dirp)) != RT_NULL)
			{
				...
				if(fp)
				{
					if(0 == nextfileflag)
					{
						while(NULL != fgets(buff,(CLIENT_RECORD_HEAD+CLIENT_RECORD_ECU_HEAD+CLIENT_RECORD_INVERTER_LENGTH*MAXINVERTERCOUNT+CLIENT_RECORD_OTHER),fp))  //¶ÁÈ¡Ò»ÐÐÊý¾Ý
						{
							if(strlen(buff) > 18)
							{
								if((buff[strlen(buff)-3] == ',') && (buff[strlen(buff)-18] == ',') )
								{
									if(buff[strlen(buff)-2] == sendflag)			//¼ì²â×îºóÒ»¸ö×Ö½ÚµÄresendflagÊÇ·ñÎª1
									{
										...
										rt_hw_ms_delay(1);
										while(NULL != fgets(buff,(CLIENT_RECORD_HEAD+CLIENT_RECORD_ECU_HEAD+CLIENT_RECORD_INVERTER_LENGTH*MAXINVERTERCOUNT+CLIENT_RECORD_OTHER),fp))	//ÔÙÍùÏÂ¶ÁÊý¾Ý£¬Ñ°ÕÒÊÇ·ñ»¹ÓÐÒª·¢ËÍµÄÊý¾Ý
										{
											if(strlen(buff) > 18)
											{
												if((buff[strlen(buff)-3] == ',') && (buff[strlen(buff)-18] == ',') )
...
{
	int i, j, count;		//EMA·µ»Ø¶àÉÙ¸öÊ±¼ä(ÓÐ¼¸¸öÊ±¼ä,¾ÍËµÃ÷EMA±£´æÁË¶àÉÙÌõ¼ÇÂ¼)
...
int detection_control_resendflag2(void)		//´æÔÚ·µ»Ø1£¬²»´æÔÚ·µ»Ø0
{
	DIR *dirp;
...
int change_control_resendflag(char *time,char flag)  //¸Ä±ä³É¹¦·µ»Ø1£¬Î´ÕÒµ½¸ÃÊ±¼äµã·µ»Ø0
{
	DIR *dirp;
	char dir[30] = "/home/record/ctldata/";
	struct dirent *d;
		...
		else
		{
			...
			while ((d = readdir(dirp)) != RT_NULL)
			{
				...
				if(fp)
				{
					if(0 == nextfileflag)
					{
						while(NULL != fgets(buff,(CONTROL_RECORD_HEAD + CONTROL_RECORD_ECU_HEAD + CONTROL_RECORD_INVERTER_LENGTH * MAXINVERTERCOUNT + CONTROL_RECORD_OTHER),fp))  //¶ÁÈ¡Ò»ÐÐÊý¾Ý
						{
							if(strlen(buff) > 18)
							{
								if((buff[strlen(buff)-3] == ',') && (buff[strlen(buff)-18] == ',') )
								{
									if(buff[strlen(buff)-2] == sendflag)			//¼ì²â×îºóÒ»¸ö×Ö½ÚµÄresendflagÊÇ·ñÎª1
									{
										...
										//print2msg(ECU_DBG_CLIENT,"search_readflag data",data);
										rt_hw_ms_delay(1);
										while(NULL != fgets(buff,(CONTROL_RECORD_HEAD + CONTROL_RECORD_ECU_HEAD + CONTROL_RECORD_INVERTER_LENGTH * MAXINVERTERCOUNT + CONTROL_RECORD_OTHER),fp))	//ÔÙÍùÏÂ¶ÁÊý¾Ý£¬Ñ°ÕÒÊÇ·ñ»¹ÓÐÒª·¢ËÍµÄÊý¾Ý
										{
											if(strlen(buff) > 18)
											{
												if((buff[strlen(buff)-3] == ',') && (buff[strlen(buff)-18] == ',') )
...
//´´½¨±¨¾¯ÐÅÏ¢	mos_status×´Ì¬Í¨¹ýÊä³öµçÑ¹ÅÐ¶Ï¡£Êä³öµçÑ¹´óÓÚ0 ±íÊ¾MOS¹Ü¿ªÆô£¬Êä³öµçÑ¹Ð¡ÓÚ0 ±íÊ¾MOS¹Ü¹Ø±Õ¡£
void create_alarm_record(inverter_info *inverter)
{
	...
	for(i = 0;i<ecu.validNum;i++)
	{
		if(curinverter->status.comm_status == 1)
		{
			...
			{
				//´æÔÚÓë×îºóÒ»ÂÖ²»Í¬µÄ×´Ì¬£¬ÐèÒªÉú³É×´Ì¬¸æ¾¯ÐÅÏ¢
				create_flag = 1;
				num++;
			}
		}
...
int detection_alarm_resendflag2(void)		//´æÔÚ·µ»Ø1£¬²»´æÔÚ·µ»Ø0
{
	DIR *dirp;
...
int change_alarm_resendflag(char *time,char flag)  //¸Ä±ä³É¹¦·µ»Ø1£¬Î´ÕÒµ½¸ÃÊ±¼äµã·µ»Ø0
{
	DIR *dirp;
	char dir[30] = "/home/record/almdata/";
	struct dirent *d;
...
int read_line(char* filename,char *linedata,char* compareData,int len)
{
  ...
  while(fgets(linedata,100,fin))//ä»ŽåŽŸæ–‡ä»¶è¯»å–ä¸€è¡Œ
	{
		if(!memcmp(linedata,compareData,len))
		{
			//å­˜åœ¨ç›¸åŒè¡Œï¼Œå…³é—­æ–‡ä»¶   ç„¶åŽè¿”å›ž1  è¡¨ç¤ºå­˜åœ¨è¯¥è¡Œ
			fclose(fin);
			return 1;
		}
	}
