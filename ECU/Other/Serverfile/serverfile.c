...
{
    int i = 0,j = 0;
    if(1 == clientResponse.Flag)
    {
        for(i = 0;i <clientResponse.DateTime_Num;i++)
        {
            for(j=0; j<3; j++)
            {
                if(1 == change_resendflag(clientResponse.DateTime[j],'0'))
                {
        ...
        for(i = 0;i<ecu.validNum;i++)
        {
            if(curinverter->status.comm_status == 1)
            {
                if(curinverter->status.alarm_flag  == 1)
                {
                    ...
                    length += 12;
                    alarm_data[length++] = curinverter->status.turn_on_off_flag + '0';
