...
unsigned char rateOfProgress = 0;		//µ±«∞Ω¯∂»
...
int getaddrOldOrNew(char *id)
{
	...
	if ((11 == ret)
		...
		for(index=0; (index<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); index++, curinverter++)			//ÊúâÊïàÈÄÜÂèòÂô®ËΩÆËÆ≠
		{
...
void bind_inverters()
{
  int num = 0,i = 0,index = 0;
   ...
   process_channel();
   rateOfProgress = 40;
   zb_change_ecu_panid(); //Ω´ECUµƒPANID∫Õ–≈µ¿…Ë÷√≥…≈‰÷√Œƒº˛÷–µƒ

	//1.∞Û∂®“—æ≠”–∂Ãµÿ÷∑µƒƒÊ±‰∆˜,»Á∞Û∂® ß∞‹£¨‘Ú–Ë“™÷ÿ–¬ªÒ»°∂Ãµÿ÷∑
	//∂‘√ø∏ˆƒÊ±‰∆˜Ω¯––∞Û∂®
	curinverter = inverterInfo;
	for(index=0; (index<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); index++, curinverter++)			//ÊúâÊïàÈÄÜÂèòÂô®ËΩÆËÆ≠
	{
	...
	if(num>0)
	{
		...
		for(i=0;i<5;i++)
		{
			...
			for(index=0; (index<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); index++, curinverter++)			//ÊúâÊïàÈÄÜÂèòÂô®ËΩÆËÆ≠
			{
				if((curinverter->shortaddr == 0) && (curinverter->status.bindflag == 0))
				{
					zb_change_inverter_channel_one(curinverter->uid,ecu.channel);//ÊâÄÊúâÈÄÜÂèòÂô®ËÆæÁΩÆÊàê0xFFFF
