...
int getaddrOldOrNew(char *id)
{
	...
	if ((11 == ret)
		...
		for(index=0; (index<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); index++, curinverter++)			//有效逆变器轮训
		{
	...
	if(num>0)
	{
		...
		for(i=0;i<5;i++)
		{
			...
			for(index=0; (index<MAXINVERTERCOUNT)&&(12==strlen(curinverter->uid)); index++, curinverter++)			//有效逆变器轮训
			{
				if((curinverter->shortaddr == 0) && (curinverter->status.bindflag == 0))
				{
					zb_change_inverter_channel_one(curinverter->uid,ecu.channel);//所有逆变器设置成0xFFFF
