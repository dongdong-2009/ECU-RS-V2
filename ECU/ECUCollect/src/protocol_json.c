...
void protocol_SF(cJSON *ecu_data_root)
{
	int i = 0, j = 0;
	int SFJsonFlag = 0;
	...
	for(i=0; (i<MAX_THIRD_INVERTER_COUNT)&&((int)strlen(curThirdinverter->inverterid) > 0);
	    i++, curThirdinverter++)
	{
		if(!memcmp("SF001",curThirdinverter->factory,5) && 
		   curThirdinverter->third_status.communication_flag == 1) //能通讯上
		{
		    ...
		    if(0 == SFJsonFlag)
	        {
	            cJSON_AddItemToObject(ecu_data_root, "SF001", inverter_addecuid_root = cJSON_CreateObject());
			...
			sprintf(buff,"SF%s",curThirdinverter->inverterid);
...
void protocol_HW(cJSON *ecu_data_root)
{
	int i = 0, j = 0;
	int HWJsonFlag = 0;
	char cPVch = '0', cACch = '0'; //标志CH的字符
	...
	for(i=0; (i<MAX_THIRD_INVERTER_COUNT)&&((int)strlen(curThirdinverter->inverterid) > 0);
	    i++, curThirdinverter++)
	{
		if(!memcmp("HW001",curThirdinverter->factory,5) && 
		   curThirdinverter->third_status.communication_flag == 1) //能通讯上
		{
            cJSON *inverter_data_root, *Alternating_Current_root,
                  *channel_data_root, *channel_data,*Alternating_Current;
		    ...
		    if(0 == HWJsonFlag)
	        {
	            cJSON_AddItemToObject(ecu_data_root, "HW001", inverter_addecuid_root = cJSON_CreateObject());
	            cJSON_AddStringToObject(inverter_addecuid_root, "EID", ecu.ECUID12);
	            cJSON_AddItemToObject(inverter_addecuid_root, "D", inverter_data_roots = cJSON_CreateArray());
			...
			cJSON_AddItemToArray(inverter_data_roots,inverter_data_root=cJSON_CreateObject());
			sprintf(buff,"HW%s",curThirdinverter->inverterid);
			cJSON_AddStringToObject(inverter_data_root, "SN", buff);
			cJSON_AddItemToObject(inverter_data_root, "P", channel_data_root = cJSON_CreateArray());
...
void protocol_SA(cJSON *ecu_data_root)
{
        ...
        i++, curThirdinverter++)
    {
                ...
                curThirdinverter->third_status.communication_flag == 1) //能通讯上
        {
            ...
            cJSON_AddItemToArray(inverter_data_roots,inverter_data_root=cJSON_CreateObject());
            sprintf(buff,"SA%s",curThirdinverter->inverterid);
            cJSON_AddStringToObject(inverter_data_root, "SN", buff);
            cJSON_AddStringToObject(inverter_data_root, "PVN","3");
            cJSON_AddItemToObject(inverter_data_root, "P", channel_data_root = cJSON_CreateArray());
...
int protocol_JSON(char *sendcommanddatetime,inverter_third_info *firstThirdinverter,char *APSString)
{
	...
	protocol_HW(ecu_data_root);
	protocol_SF(ecu_data_root);
